...
//#region node_modules/.nitro/vite/services/ssr/assets/local-extract-DSbGgCx7.js
var WEEKDAYS = {
	sunday: 0,
	monday: 1,
	tuesday: 2,
...
function extractDate(text, from) {
	...
	if (onDay && /\b(party|trip|appointment|photos?|bake|fair|swimming|after school|conferences?|dentist)\b/i.test(text)) return nextNamedDay(from, WEEKDAYS[onDay[1].toLowerCase()]);
	return null;
}
...
function classify(text) {
	...
	if (/\b(sign and return|return the|return this|due by|book your slot|pay by)\b/i.test(text) || /\bby\s+(monday|tuesday|wednesday|thursday|friday|saturday|sunday|\d)/i.test(text) && !/\b(party|trip|appointment|conferences?|parent-teacher|fair|photos?|swimming|bake sale|dentist)\b/i.test(text)) return "deadline";
	if (/\b(party|trip|appointment|conferences?|parent-teacher|fair|photos?|swimming|bake sale|museum|dentist)\b/i.test(text)) return "event";
	if (/\b(need|bring|pack|packed lunch|swimsuit|volunteers needed)\b/i.test(text)) return "task";
	return "task";
}
function tidyLead(text) {
	return text.replace(/^[•\-–]\s*/, "").replace(/^(hi|hello|hey)(\s+\w+)?[!.,]?\s*/i, "").replace(/^just confirming\s+/i, "").replace(/^you're invited to\s+/i, "").replace(/^please\s+/i, "").replace(/^reminder:\s*/i, "").replace(/^your child will\s+/i, "").replace(/^children\s+/i, "").replace(/^need (?:a |an |to )?/i, "").replace(/\s+/g, " ").trim();
}
...
function isNoise(text) {
	const t = text.trim();
	return t.length < 8 || /^(thanks|thank you|hope you can|have a good|contact:|ms\.|mr\.|from,)/i.test(t) || /newsletter$/i.test(t) || /^permission slip/i.test(t) || /^no gifts needed/i.test(t);
}
function isEnrichment(text) {
	return /^(coach leaves|sessions?\b|return approx|arrive \d)/i.test(text.trim());
}
function splitCombined(chunk) {
	const m = chunk.match(/^(.*?)(?:[.!]\s+|\s+[—–]\s+)((?:please\s+)?(?:return the|sign and return|book your slot|rsvp\b|reply yes|pay by|volunteers needed).+)$/i);
...
function leftoverNotes(chunk, title) {
	...
	extra = extra.replace(/^(hi|hello|hey)(\s+\w+)?[!.,]?\s*/i, "").replace(/^you're invited to\s+/i, "").replace(/^just confirming\s+/i, "").replace(/^please\s+/i, "").replace(/^reminder:\s*/i, "").replace(/^[•\-–.!,;:\s]+/, "").trim();
...
function localExtract(text, from = /* @__PURE__ */ new Date()) {
	...
	for (const chunk of chunks) {
		...
		if (!(Boolean(date) || Boolean(time) || type === "rsvp" || type === "deadline" || /\b(need|bring|pack|appointment|trip|party|photos|swimming|conference|bake|fair|dentist|swimsuit|volunteers)\b/i.test(chunk))) continue;
		seen.add(key);
		items.push({
			type,
			title,
